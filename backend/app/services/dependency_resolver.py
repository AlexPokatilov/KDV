...
def build_graph(namespace: str, k8s: KubernetesClient) -> GraphResponse:
    ...
    for dep in deployments:
        ...
        for pod in pods:
            if _labels_match(selector, pod.metadata.labels or {}):
                add_edge(
                    dep_id, _node_id("Pod", namespace, pod.metadata.name), "selects"
                )
    ...
    for sts in statefulsets:
        ...
        for pod in pods:
            if _labels_match(selector, pod.metadata.labels or {}):
                add_edge(
                    sts_id, _node_id("Pod", namespace, pod.metadata.name), "selects"
                )
    ...
    for ds in daemonsets:
        ...
        for pod in pods:
            if _labels_match(selector, pod.metadata.labels or {}):
                add_edge(
                    ds_id, _node_id("Pod", namespace, pod.metadata.name), "selects"
                )
    ...
    for job in jobs:
        ...
        for pod in pods:
            if _labels_match(selector, pod.metadata.labels or {}):
                add_edge(
                    job_id, _node_id("Pod", namespace, pod.metadata.name), "selects"
                )
    ...
    for svc in services:
        ...
        for pod in pods:
            if _labels_match(selector, pod.metadata.labels or {}):
                add_edge(
                    svc_id, _node_id("Pod", namespace, pod.metadata.name), "selects"
                )
    ...
    for pvc in pvcs:
        if pvc.spec.volume_name and pvc.spec.volume_name in pv_by_name:
            add_edge(
                pvc_by_name[pvc.metadata.name],
                pv_by_name[pvc.spec.volume_name],
                "bound-to",
            )
